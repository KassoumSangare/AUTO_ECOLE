...
@section('content')
    ...
    {{-- Alertes --}}
    @if(session('success'))
        <div class="alert alert-success d-flex align-items-center gap-2 mb-3" role="alert" style="border-radius:10px;">
    ...
    <div class="card-a mb-3">
        <div class="row g-2 align-items-end">
            ...
            <div class="col-md-4">
                ...
                <div class="search-wrap">
                    ...
                    <input type="text" id="searchInput" class="search-in" placeholder="Nom, prénom, téléphone…"
            ...
            <div class="col-md-6">
                <form method="GET" class="d-flex gap-2 flex-wrap align-items-end">
                    <div>
                        <label
                            style="font-size:.76rem;font-weight:700;color:var(--texte);display:block;margin-bottom:.3rem;">Du</label>
                        <input type="date" name="date_debut" value="{{ request('date_debut') }}"
                            class="form-control form-control-sm" style="border-radius:8px;border-color:var(--border);">
                    </div>
                    <div>
                        <label
                            style="font-size:.76rem;font-weight:700;color:var(--texte);display:block;margin-bottom:.3rem;">Au</label>
                        <input type="date" name="date_fin" value="{{ request('date_fin') }}"
                            class="form-control form-control-sm" style="border-radius:8px;border-color:var(--border);">
                    </div>
                    <div>
                        <label
                            style="font-size:.76rem;font-weight:700;color:var(--texte);display:block;margin-bottom:.3rem;">Statut</label>
        ...
        @if(request()->hasAny(['date_debut', 'date_fin', 'statut']))
            <div class="mt-2 d-flex gap-2 flex-wrap">
                @if(request('date_debut'))<span class="filter-chip"><i class="bi bi-calendar3"></i>Depuis
                {{ request('date_debut') }}</span>@endif
                @if(request('date_fin'))<span class="filter-chip"><i class="bi bi-calendar3"></i>Jusqu'au
                {{ request('date_fin') }}</span>@endif
                @if(request('statut'))<span class="filter-chip"><i
                class="bi bi-funnel"></i>{{ request('statut') === 'paye' ? 'Payés' : 'Non payés' }}</span>@endif
            </div>
        @endif
    </div>
    ...
    <div class="card-a">
        <div class="d-flex align-items-center justify-content-between mb-3">
            ...
            <span style="font-size:.78rem;color:var(--texte-2);">Page
                {{ $eleves->currentPage() }}/{{ $eleves->lastPage() }}</span>
        </div>
            ...
            <table class="table tbl mb-0" id="elevesTable">
                ...
                <tbody id="tableBody">
                    @forelse($eleves as $eleve)
                        <tr class="eleve-row"
                            data-search="{{ strtolower($eleve->nom . ' ' . $eleve->prenom . ' ' . $eleve->telephone) }}">
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <div class="av">{{ strtoupper(substr($eleve->prenom, 0, 1)) }}</div>
                                    <div>
                                        <div style="font-weight:700;color:var(--texte);">{{ strtoupper($eleve->nom) }}
                                            {{ $eleve->prenom }}</div>
                            ...
                            {{-- Colonne Paiement Wave --}}
                            <td>
                                @if($eleve->payments->count())
                                    <span class="b-paye"><i class="bi bi-check2-circle me-1"></i>Wave ✓</span>
                                @else
                                    <span class="b-nopaye"><i class="bi bi-clock me-1"></i>En attente</span>
                                @endif
                            </td>

                            {{-- Colonne Accès Manuel (NOUVEAU) --}}
                            <td>
                                <form action="{{ route('admin.eleves.toggle-acces', $eleve) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit"
                                        class="btn btn-sm {{ $eleve->is_manually_approved ? 'btn-outline-danger' : 'btn-outline-primary' }}"
                                        style="border-radius:8px;font-size:.75rem;"
                                        onclick="return confirm('Modifier l\'accès de {{ addslashes($eleve->prenom) }} ?')">
                                        @if($eleve->is_manually_approved)
                                            <i class="bi bi-lock-fill me-1"></i>Révoquer
                                        @else
                                            <i class="bi bi-unlock-fill me-1"></i>Accorder
                                        @endif
                                    </button>
                                </form>
                            </td>
                            ...
                            <td>
                                <span
                                    style="font-family:'Syne',sans-serif;font-weight:800;color:var(--texte);">{{ $eleve->documents_count }}</span>
                            </td>
                            <td>
                                <span
                                    style="font-family:'Syne',sans-serif;font-weight:800;color:var(--texte);">{{ $eleve->quiz_scores_count }}</span>
                            </td>
                            <td>
                                <a href="{{ route('admin.eleves.show', $eleve) }}" class="btn btn-sm"
