...
@section('content')
    ...
    @if(!$stats['has_paid'])
        <div class="payment-banner mb-4">
            <div style="position:relative;z-index:1;">
                ...
                <p style="opacity:.82;font-size:.9rem;margin-bottom:1rem;">Effectuez votre paiement pour accéder aux vidéos,
                    quiz et au coffre numérique.</p>
                <a href="{{ route('eleve.payment') }}" class="btn fw-bold"
                    style="background:#fff;color:var(--rouge);border-radius:10px;">
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-lg-3">
            <div class="kstat">
                <div class="ico" style="background:var(--rouge-p);color:var(--rouge);"><i class="bi bi-folder2-open"></i>
                </div>
        ...
        <div class="col-6 col-lg-3">
            <div class="kstat">
                <div class="ico" style="background:var(--rouge-p);color:var(--rouge);"><i
                        class="bi bi-patch-question-fill"></i></div>
        ...
        <div class="col-6 col-lg-3">
            <div class="kstat">
                <div class="ico"
                    style="background:{{ $stats['has_paid'] ? 'var(--vert-p)' : 'var(--rouge-p)' }};color:{{ $stats['has_paid'] ? 'var(--vert)' : 'var(--rouge)' }};">
                    <i class="bi bi-{{ $stats['has_paid'] ? 'patch-check-fill' : 'clock' }}"></i>
                </div>
                <div class="val" style="font-size:1.1rem;padding-top:.4rem;">{{ $stats['has_paid'] ? 'Actif' : 'En attente' }}
                </div>
        ...
        <div class="col-lg-6">
            <div class="kstat">
                ...
                <div class="mb-3">
                    ...
                    <div class="prog-bar">
                        <div class="prog-fill" style="width:0%;background:var(--rouge);"
                            data-width="{{ $stats['quiz_code'] }}"></div>
                    </div>
                </div>
                <div>
                    <div class="d-flex justify-content-between mb-1">
                    ...
                    <div class="prog-bar">
                        <div class="prog-fill" style="width:0%;background:var(--vert);"
                            data-width="{{ $stats['quiz_cond'] }}"></div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="kstat">
                ...
                @forelse($stats['last_scores'] as $score)
                    <div class="score-row">
                        <div>
                            <span class="badge"
                                style="background:{{ $score->category === 'code' ? 'var(--rouge-p)' : 'var(--vert-p)' }};color:{{ $score->category === 'code' ? 'var(--rouge-c)' : 'var(--vert-c)' }};font-size:.75rem;">
                                {{ ucfirst($score->category) }}
                            </span>
                            <small class="text-muted ms-2">{{ $score->created_at->diffForHumans() }}</small>
                        </div>
                        <div class="fw-bold"
                            style="color:{{ $score->is_reussi ? 'var(--vert)' : 'var(--rouge)' }};font-family:'Syne',sans-serif;">
